...
def test_extract_toc_indices_with_mocked_gemini(monkeypatch):
    ...
    # Mock call_gemini to return titles that appear twice in text
    with patch("ai_text_outline._extract.call_gemini") as mock_gemini:
        mock_gemini.return_value = {"Chapter 1": 1, "Chapter 2": 2}

        # Text where "Chapter 1" and "Chapter 2" appear twice
...
def test_extract_toc_indices_title_appears_once(monkeypatch):
    ...
    with patch("ai_text_outline._extract.call_gemini") as mock_gemini:
        mock_gemini.return_value = {"Unique Title": 1}
...
"""
        ...
        result = extract_toc_indices(text=text)
        # Title appears only once, so no index is added
        assert result == []
...
"""
    ...
    with patch("ai_text_outline._extract.call_gemini") as mock_gemini:
        mock_gemini.return_value = {"Chapter A": 1, "Chapter B": 2}
